      ...
      tapCallback:(void(^)(void))tapCallback
{
    @try {
        ...
        toast.center = [bgView centerPointForPosition:position withToast:toast];
        toast.alpha = 0.0;
...
- (void)hideToast:(UIView *)toast animated:(BOOL)animated {
    if (animated) {
...
- (void)handleToastTapped:(UITapGestureRecognizer *)recognizer {
    UIView *toastView = [[recognizer.view subviews] firstObject];
    
    NSTimer *timer = (NSTimer *)objc_getAssociatedObject(toastView, &CSToastTimerKey);
    
    [timer invalidate];
    
    void (^callback)(void) = objc_getAssociatedObject(toastView, &CSToastTapCallbackKey);
    if (callback) {
        callback();
    }
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding + offsetY);
